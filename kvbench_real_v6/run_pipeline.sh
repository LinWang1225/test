...
read -r -a LEVELS <<< "${PILOT_CONCURRENCY:-8}"
COMMON=(--data-dir "$DATA" --apc "${APC:-0}" --gpu "${GPU:-0}"
        --max-seqs "${MAXSEQ:-16}" --max-len "${MAXLEN:-32768}")
# By default do not send request-level max_tokens. Optional debug caps are opt-in only.
[[ -z "${MATH_MAX_TOKENS:-}" ]] || COMMON+=(--math-max-tokens "$MATH_MAX_TOKENS")
[[ -z "${HUMAN_MAX_TOKENS:-}" ]] || COMMON+=(--human-max-tokens "$HUMAN_MAX_TOKENS")
