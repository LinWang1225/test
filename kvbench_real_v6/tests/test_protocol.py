...
def config():
    return dict(methods=['fp16','tq4','kvarn'],datasets=['math_500','humaneval'],stage='pilot',pilot_size=32,
        ...
        math_max_tokens=None,human_max_tokens=None,output_limit_policy='server_context_window_only',kv_bytes=None,gpu_util=.85,thinking=True,
        ...
        max_pilot_cap_fraction=None,max_preemptions=-1,max_p95_ttft_ms=None,max_p95_tpot_ms=None)
...
class Tests(unittest.TestCase):
    def test_natural_request_no_answer_leak(self):
        ...
        self.assertNotIn('max_tokens',r[0]['request']);self.assertNotIn('ignore_eos',r[0]['request'])
        self.assertNotIn('min_tokens',r[0]['request']);self.assertNotIn('stop',r[0]['request']);self.assertEqual(len(r),32)
    ...
    def test_warmup_no_matching_prefix(self):
        ...
        self.assertNotIn('max_tokens',warm[0]['request']);self.assertNotIn('ignore_eos',warm[0]['request'])

    def test_context_not_silently_truncated(self):
        c=config();c['max_len']=16
        p={'number':2,'id':'x','repeat':1,'max_tokens':None}
        class Tok:
    ...
    def test_truncation_retained(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            s=export_run(root/'measured',root,req,True)
            self.assertTrue(s['measurement_complete']);self.assertEqual(s['length_limit_terminations'],3)
            self.assertTrue(eligible(s,config())[0])
            c=config();c['max_pilot_cap_fraction']=.05;self.assertFalse(eligible(s,c)[0])
    ...
    def test_concurrency_slo_not_oom_only(self):
        ...
        s={'measurement_complete':True,'length_capped_fraction':0,'missing_finish_reason':0,'token_usage_missing':0,'tpot_ms_p95':101}
