...
- 自然停止：`ignore_eos=false`、`min_tokens=0`、没有人为添加的字符串 stop（请求中的 stop=[]）；保留安全上限：MATH 16384、HumanEval 8192。长度上限不是目标长度。finish_reason=length 单列，仍保留输出并评分，不伪称自然完成。
...
它顺序完成：真实题 pilot → 选择各方法共同可用的并发 → 三种方法的 MATH-500/HumanEval 全集自然生成 → 对保存输出进行 MATH 评分。无可用并发建议时停止，不自动改生成上限或精度。
...
可覆盖的管线起始参数：`PILOT_CONCURRENCY='2 4 8 16'`、`PILOT_SIZE=32`、`MAXSEQ=16`、`MAXLEN=32768`、`MATH_MAX_TOKENS=16384`、`HUMAN_MAX_TOKENS=8192`、`APC=0`、`GPU=0`、`KV_BYTES=...`。pipeline 不覆盖已有结果目录；中断后按下面的分阶段命令加 `--resume` 继续。
...
默认 pilot 候选要求：正式请求齐全且成功、原生 usage 与 finish reason 齐全、达到长度上限的比例不超过 5%。5% 是本包的**可修改筛查阈值**，不是自然结束或统计显著性的保证。比例过高应统一提高同一数据集各方法的生成上限并重新测，而不是删掉这些题。
...
遇到失败时保存证据；开启 continue-on-error 后，跳过同组更高并发，继续其他方法/数据集。不会为了跑完而自动改位宽、输出上限、显存预算或 offload。pilot 建议是小样本经验结果，正式长尾请求仍可能失败，需要以新目录显式降低并发再测。
...
检查模型、数据、硬件、包、源码、输出上限、采样与缓存预算是否与 pilot 一致；不一致时拒绝套用建议。没有可用建议时也不会擅自选并发。
...
每点单独预热，不计入正式统计；预热使用真实题目，但添加不匹配的 leading system 标识，避免预载正式前缀。预热也允许自然结束，使用相同安全上限；完整预热输出另外保存。默认 2 个预热请求并不保证所有动态批形状都已编译，可增加 `--warmup-requests`。因此正式比较要查看首批延迟并对关键点复测。

达到上限并不使测量结果消失，状态仍可为测量完成；`length_capped` 单列。请求失败/缺失、生成参数被覆盖、输出无法导出不会标为完成。没有 reasoning parser 时，Qwen3 thinking 输出应以闭合 `</think>` 后的内容供评分；没有闭合且没有服务端 reasoning channel 时保留全文，并将“最终答案未提取”单列，而不是把未完成推理中的一个答案当最终答案。
...
结构化字段区分请求配置与原生日志值：
- 配置：方法、KV dtype、block_size、APC 开关、kv_cache_memory_bytes_requested、gpu_memory_utilization_requested、引擎参数、实际客户端并发、输出上限和采样参数。
...
先看数据/请求是否齐全、自然停止比例、输出上限触发率，再看质量与时延。相同任务/采样/并发的结果放在一张表；每种方法各自较优并发的结果另做部署容量表。
