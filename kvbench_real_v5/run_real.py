...
def parser():
    ...
    p.add_argument('--math-max-tokens',type=int,default=16384);p.add_argument('--human-max-tokens',type=int,default=8192)
    ...
    p.add_argument('--max-pilot-cap-fraction',type=float,default=0.05)
...
def compatibility(cfg):
    keys=['model','methods','datasets','gpu_util','kv_bytes','max_len','max_seqs','batch_tokens','eager',
          'math_max_tokens','human_max_tokens','thinking','temperature','top_p','top_k','seed','data_seed',
            ...
            for dataset,apc in itertools.product(cfg['datasets'],cfg['apc']):
                ...
                for c in levels:
                    ...
                    p=dict(id=f'r{rep:02d}_{method}_{dataset}_apc{apc}_C{c}',repeat=rep,method=method,
                        dataset=dataset,apc=apc,concurrency=c,number=n,max_tokens=cap,
                        ...
                        seed_base=cfg['seed'],ignore_eos=False,min_tokens=0,stop=[],eager=cfg['eager'])
                    points.append(p)
    return points
...
def build_requests(data,p,cfg,tokenizer=None,warm=False):
    ...
    for i,row in enumerate(selected):
        ...
        req={'model':'qwen-kvbench','messages':messages,'stream':True,'stream_options':{'include_usage':True},
             'n':1,'repetition_penalty':1.0,'frequency_penalty':0.0,'presence_penalty':0.0,'max_tokens':p['max_tokens'],'min_tokens':0,'ignore_eos':False,'stop':[],
        ...
        if tokenizer is not None:
            ...
            if tok_count+p['max_tokens']>cfg['max_len']:
                raise ValueError(f'{row["case_id"]}: prompt {tok_count} + cap {p["max_tokens"]} > window {cfg["max_len"]}; 不截断题目。')
...
def eligible(stat,cfg):
    ...
    if (stat.get('length_capped_fraction') or 0)>cfg['max_pilot_cap_fraction']:reasons.append('too_many_length_capped_requests')
...
def recommend(root,cfg,records,info):
    ...
    atomic_json(root/'recommendations.json',{'compatibility':compatibility(cfg),'source_identity':info,'choices':choices,
        'selection_objective':'successful requests/s subject to observed completion/cap/SLO criteria; accuracy not used until offline grading',
...
class NaturalRunner(ServerManager):
    ...
    def one(self,p,dest):
        ...
        try:
            ...
            log(f"完成 {p['id']}：{stat['succeeded']} 请求，RPS={stat['rps']:.4f}，输出均值={stat['output_tokens_avg']:.1f}，达到上限={stat['length_capped']}")
...
def configure(a):
    ...
    for key in ['pilot_size','repeats','max_len','max_seqs','batch_tokens','math_max_tokens','human_max_tokens','warmup_requests','request_timeout','point_timeout','startup_timeout']:
    ...
    if not 0<=cfg['max_pilot_cap_fraction']<=1:raise ValueError('max-pilot-cap-fraction 必须在 [0,1]')
...
def main():
    ...
    for p in plan:print(f"  {p['id']} n={p['number']} max_tokens={p['max_tokens']} APC={p['apc']}")
